...
/*float findLeafValue2(Node* pointLeaf, TableHand *handTable){
...
Node* PowerRanger2(Hand* hand, TableHand *handTable, CombinationTree* tree){

    Node** leaves = tree->getLeaves();

    for(int i=0; i < tree->getNumberOfleaves(); i++){
       float temp = findLeafValue2(leaves[i], handTable);
...
int ComputerPlayer::decideAndPlay(Combination** playableCombinations, int numPlayableCombinations, Table* table, PlayerStatus* playerStatuses, int* numberOfCardsPerPlayer, Combination* lastCombinationOnTable){
    ...
    decky.HandCard(hand);
    TableHand* handy = new TableHand(decky.cardplayed, decky.specialcardplayed);
    Node* newNode = PowerRanger2(hand, handy, tree);
    */
    //cout<<endl<<endl;
    //cout<<"------COUNTER IS---------"<<tree->getCounter()<<endl;
    if (lastCombinationOnTable != NULL){
